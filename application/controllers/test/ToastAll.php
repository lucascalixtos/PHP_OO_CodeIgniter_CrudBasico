...
// para grandes suites de teste, elimine o limite de tempo
set_time_limit(0);
...
class ToastAll extends MY_Controller
{
	...
	function index($mod = null){
		$output = '';
		
		// Fetch all test classes
        $test_files = $this->_get_test_files($mod);
        $aux = $mod ? $mod.'/' : '';
		...
		foreach ($test_files as $file) {
			$test_urls[] = site_url($aux . $this->test_dir . $file . '/show_results');
        }
	 ...
	 */
	function _get_test_files($mod){
        $files = array();        
        $aux = $mod ? "modules/$mod" : '';
		$handle=opendir(APPPATH . $aux . '/controllers/' . $this->test_dir);
